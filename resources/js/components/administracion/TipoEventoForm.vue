...
interface FormData {
    ...
    codigo: string;
    descripcion: string;
    tipo_calendarizacion_id: number | null;
}

const props = defineProps(['item', 'accion', 'tiposCalendarizacion']);

const formData = ref<FormData>({
    id: null,
    codigo: '',
    descripcion: '',
    tipo_calendarizacion_id: null,
});
...
async function submitForm() {
    ...
    if (valid) {
        try {
            const resp = await axios.post(route('calendarizacion-tipo_evento-save'), formData.value);
...
const titleForm = computed(() => {
    return isEditing.value ? t('tipoEvento._editar_') : t('tipoEvento._crear_');
});
</script>
<template>
                ...
                <v-row>
                    <v-col cols="12">
                        ...
                        <v-text-field
                            ...
                            :rules="[rules.required, rules.maxLength(155)]"
                            counter="155"
                        ...
                        <v-autocomplete
                            ...
                            :label="$t('tipoCalendarizacion._singular_') + ' *'"
                            :items="tiposCalendarizacion"
                            v-model="formData.tipo_calendarizacion_id"
                            item-title="codigo"
                            item-value="id"
