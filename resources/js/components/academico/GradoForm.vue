...
async function submitForm() {
    ...
    if (valid) {
        try {
            const resp = await axios.postForm(route('academico-plan_estudio-grado-save'), formData.value);
...
const titleForm = computed(() => {
    return isEditing.value ? t('grado._editar_grado_') : t('grado._crear_grado_');
});
</script>
<template>
